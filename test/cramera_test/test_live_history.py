...
class TestMotionHistoryPublication:
    ...
    def test_native_reset_clears_motion_progress_during_active_plan(
        self, motion_execution: MotionExecution
    ) -> None:
        """
        A reset motion clears its progress while its enclosing plan remains active.
        """
        ...
        with motion_execution.plan.root.execution_scope():
            motion_execution.record(LifeCycleValues.RUNNING)
            ...
            assert motion_execution.motion.start_time is None
            assert motion_execution.motion.end_time is None
            assert motion_execution.plan.root.completed_children_status is None
