...
# %% cancelled execution
def test_cancelled_tick_does_not_publish_incomplete_snapshot(mini_world) -> None:
    """
    An immediate cancellation retains only the preceding complete control cycles.
    """
    ...
    chart.tick(context)
    completed = list(recorder.snapshots)

    with pytest.raises(RuntimeError) as caught:
        chart.tick(context)

    assert caught.value is failure
    assert recorder.snapshots == completed
    assert chart.history.history == completed
...
def test_cancellation_prevents_later_node_callbacks(mini_world, monkeypatch) -> None:
    """
    No later observation callback can mutate the world after cancellation.

    :param mini_world: World used by the native statechart.
    :param monkeypatch: Restore the observed callback after the test.
    """
    failure = RuntimeError("motion cancelled")
    chart = MotionStatechart()
    chart.add_node(CancelMotion(exception=failure))
    later_node = MotionStatechartNode()
    chart.add_node(later_node)
    callback = Mock(return_value=None)
    monkeypatch.setattr(later_node, "on_tick", callback)
    context = MotionStatechartContext(world=mini_world)
    chart.compile(context)
    chart.tick(context)
    completed = list(chart.history.history)
