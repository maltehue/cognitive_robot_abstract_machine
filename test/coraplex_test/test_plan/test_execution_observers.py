...
def test_native_reset_clears_collapsed_ancestors_inside_an_active_plan() -> None:
    """A retry reopens derived action progress while the outer execution stays active."""
    plan = Plan()
    root = PlanNode()
    collapsed = PlanNode()
    node = MotionNode(designator=MoveJointsMotion(names=[], positions=[]))
    plan.add_edge(root, collapsed)
    plan.add_edge(collapsed, node)
    motion = RecordedMotion()
    observer = MotionPlanHistory(motion.chart, {node: motion.task})

    with root.execution_scope():
        motion.record(LifeCycleValues.SUCCEEDED)
        assert collapsed.status == LifeCycleValues.SUCCEEDED
        assert collapsed.end_time is not None

        motion.record(LifeCycleValues.NOT_STARTED)

        assert root.status == LifeCycleValues.RUNNING
        assert collapsed.status == LifeCycleValues.NOT_STARTED
        assert collapsed.start_time is None
        assert collapsed.end_time is None
        assert node.status == LifeCycleValues.NOT_STARTED
